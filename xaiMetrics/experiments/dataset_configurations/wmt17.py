...
if __name__ == '__main__':
    ...
    dataset_name = "wmt17"  # Tag to save explanations under
    ...
    def reader(lp):
        # Slightly inefficient to load this dataframe multiple times, but this is only 7 times
        df = pd.read_csv(os.path.join(ROOT_DIR, "xaiMetrics", "data", dataset_name, "wmt17_seg.tsv"), delimiter='\t')
        df = df[df["LP"] == lp]
        orig_len = len(df)
        df = df[df['SRC'].notna()]
        print("Dropped values:", orig_len - len(df), "lp:", lp)
        return df
